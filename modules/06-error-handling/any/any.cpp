...
int main() {
  ...
  data = string("Mark");  // allocates 32 bytes

  cout << "\nUse any_cast to get the data as a string and copy it to a new string...\n";
  const auto copied = any_cast<string>(data);
  cout << copied << endl;

  cout << "\nCast as a string reference, without copies...\n";
  const auto& referenced = any_cast<string&>(data);
  ...
  BigStruct& /*auto&*/ big_struct = any_cast<BigStruct&>(data); // NOLINT(*-use-auto)
  for (const long long i : big_struct.v) { cout << i << " "; }
  cout << endl;
}
